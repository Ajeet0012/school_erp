    ...
    useEffect(() => {
        const fetchExams = async () => {
            try {
                ...
                toast.error('Failed to synchronize academic assessment nodes.');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-12">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <Badge variant="outline" className="text-[8px] font-black uppercase tracking-widest border-2 mb-2 text-primary">
                        FACULTY_CORE: ACADEMIC_ASSESSMENT
                    </Badge>
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Exam Management Registry</h1>
                    <p className="text-sm font-medium text-slate-500 italic">Manage examination schedules, invigilation protocols, and academic result commitment nodes.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: 'Pending Assessments', value: '12', icon: <Clock className="text-amber-500" />, trend: 'CRITICAL' },
                    { label: 'Evaluation Ratio', value: '94.2%', icon: <Activity className="text-emerald-500" />, trend: 'OPTIMAL' },
                    { label: 'Average Node Score', value: '78.4', icon: <TrendingUp className="text-primary" />, trend: '+2.4%' },
                ].map((stat, i) => (
                            ...
                            exams.map((exam) => (
                                <div key={exam.id} className="group bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 border-slate-50 dark:border-slate-800 p-8 hover:border-primary/30 hover:shadow-2xl transition-all relative overflow-hidden flex flex-col justify-between cursor-pointer">
                                    ...
                                    <div className="relative z-10 flex items-start justify-between mb-6">
                                        ...
                                        <Badge
                                            variant="outline"
                                            className={`text-[7px] font-black uppercase tracking-widest border-lg px-2 py-1 ${exam.status === 'PUBLISHED' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' : 'bg-amber-500/10 text-amber-600 border-amber-500/20'}`}
                                        >
                                            {exam.status}
                                        </Badge>
                                    </div>
                                    ...
                                    <div className="relative z-10 flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Protocol Sync: ACTIVE</span>
                                        </div>
                                        <button className="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-2 hover:gap-3 transition-all group-hover:translate-x-1">
                                            Enter Marks
                                            <ArrowRight size={14} />
                                        </button>
                                    </div>
